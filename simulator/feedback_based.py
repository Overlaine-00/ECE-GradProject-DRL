...
class FeedbackSimulator:
    def __init__(self, dataset_params: dict, device: torch.device):
        ...
        ratings = ratings.set_index(['user_id', 'item_id'])
        self.ratings: dict[np.int32, pd.DataFrame] = {
            id: ratings.loc[id, :] for id in self.users
        }
    ...
    def reset(self, user_ids: np.ndarray[np.int32]) -> np.ndarray[np.int32]:
        ...
        for i , id in enumerate(user_ids):
            target_items = self.ratings[id].index.to_numpy()
    ...
    def step(self, action_likelihood: torch.FloatTensor) -> tuple[torch.LongTensor, torch.FloatTensor, torch.BoolTensor]:
        """
        ...
        return actions, feedback_reward, in_progress