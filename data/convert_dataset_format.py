...
def get_parent_directory() -> Union[str, bool]:
    if os.path.exists('/.dockerenv'):
        return '/workspace/data', True
    else:
        return 'C:\\Users\\cjhy2\\Desktop\\grad_project\\data', False
...
def convert_movielens_25m_to_csv(
    ...
    folder_path, using_docker = get_parent_directory()
    ...
    if rating:
        ...
        for i in range(5):
            ...
            if i < 4:
                if using_docker:
                    sample_ratings.to_csv(f'{folder_path}/movielens_25m/train_{i}.csv', index=False)
                else:
                    sample_ratings.to_csv(f'{folder_path}\\movielens_25m\\train_{i}.csv', index=False)
            else:
                if using_docker:
                    sample_ratings.to_csv(f'{folder_path}/movielens_25m/test_{i-4}.csv', index=False)
                else:
                    sample_ratings.to_csv(f'{folder_path}\\movielens_25m\\test_{i-4}.csv', index=False)
...
def convert_netflix_prize_to_csv(
    ...
    if rating:
        ...
        for i in range(1, 5):
            if using_docker:
                file_path = f'{folder_path}/netflix_prize/combined_data_{i}.txt'
            else:
                file_path = f'{folder_path}\\netflix_prize\\combined_data_{i}.txt'
        ...
        for i in range(10):
            ...
            if i < 8:
                if using_docker:
                    sample_ratings.to_csv(f'{folder_path}/netflix_prize/train_{i}.csv', index=False)
                else:
                    sample_ratings.to_csv(f'{folder_path}\\netflix_prize\\train_{i}.csv', index=False)
            else:
                if using_docker:
                    sample_ratings.to_csv(f'{folder_path}/netflix_prize/test_{i-8}.csv', index=False)
                else:
                    sample_ratings.to_csv(f'{folder_path}\\netflix_prize\\test_{i-8}.csv', index=False)
...
def convert_yahoo_to_csv(filename: str):
    ...
    if using_docker:
        file_path = f'{folder_path}/yahoo_r2/{filename}.txt'
    else:
        file_path = f'{folder_path}\\yahoo_r2\\{filename}.txt'
    ...
    if using_docker:
        ratings.to_csv(f'{folder_path}/yahoo_r2/{filename}.csv', index=False)
    else:
        ratings.to_csv(f'{folder_path}\\yahoo_r2\\{filename}.csv', index=False)
